...
public class TelaAgendamento extends javax.swing.JFrame {
    ...
    private static final Logger logger = LogManager.getLogger(TelaAgendamento.class);
    ...
    public TelaAgendamento() {
        ...
        initComponents();
        logger.info("A tela 'Agendamento' foi aberta"); //criado para gerar log na abertura da tela
    ...
    private void jButtonConsultarPetActionPerformed(java.awt.event.ActionEvent evt) {
        logger.info("Botão 'Consultar Pet' pressionado");
        try {
            ...
            logger.info("Pet selecionado para agendamento");
        } catch (Exception e) {
            logger.error("Erro ao consultar pet", e);
        }
    }

    private void jButtonAgendarActionPerformed(java.awt.event.ActionEvent evt) {
        logger.info("Botão 'Agendar' pressionado");
        String data = jTextFieldData.getText();
        String hora = jTextFieldHora.getText();

        try {
            Date date = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").parse(data + " " + hora + ":00");

            agendamento.setDatahora(date);
            gravarAgendamento();
            logger.info("Agendamento realizado com sucesso");
        } catch (Exception e) {
            logger.error("Erro ao agendar", e);
    ...
    private void atualizaTotal(BigDecimal valor) {
        logger.info("Atualizando valor total do agendamento");
        try {
            ...
            logger.info("Valor total atualizado com sucesso");
        } catch (Exception e) {
            logger.error("Erro ao atualizar valor total do agendamento", e);
        }
    }

    private void insereServico(Servico servico) {
        logger.info("Inserindo serviço ao agendamento");
        try {
            AgendamentoServico agendServ = new AgendamentoServico(servico, agendamento, servico.getValor());

            agendamento.getListaServicos().add(agendServ);
            logger.info("Serviço inserido com sucesso");
        } catch (Exception e) {
            logger.error("Erro ao inserir serviço", e);
        }
    }

    private void gravarAgendamento() {
        logger.info("Gravando agendamento");
        try {
            AgendamentoDAO dao = new AgendamentoDAO();
            ...
            logger.error("Erro ao gravar agendamento", e);
        }
    }
